...
public class Client {
    ...
    private void detectError(String str) throws NutException {
        if (str.startsWith("ERR ")) {
            ...
            switch (arr.length) {
                case 2 -> throw new NutException(arr[1]);
                case 3 -> throw new NutException(arr[1], arr[2]);
                default -> throw new NutException();
            }
        }
    }
     ...
     * @throws IOException
     */
    protected List<String> list(String subcmd) throws IOException, NutException {
     ...
     * @throws IOException
     */
    protected List<String> list(String subcmd, String param) throws IOException, NutException {
        String[] params = {param};
     ...
     * @throws IOException
     */
    protected List<String> list(String subcmd, String[] params) throws IOException, NutException {
        if (!isConnected()) {
            return null;
        }
        subcmd = merge(subcmd, params);
    ...
    public List<Device> getDeviceList() throws IOException, NutException {
        List<String> res = list("UPS");
        if (res == null) {
            return null;
        }
        List<Device> deviceList = new ArrayList<>();
        for (int i = 0; i < res.size(); i++) {
            String[] arr = splitNameValueString(res.get(i));
            if (arr != null) {
                deviceList.add(new Device(arr[0], this));
            }
        }
        return deviceList;
    }
